...
// CHECK: @"$s4main3BoxVyxGAA1TAAWP" = hidden constant [1 x i8*] [i8* bitcast (%swift.protocol_conformance_descriptor* @"$s4main3BoxVyxGAA1TAAMc" to i8*)], align [[ALIGNMENT]]
...
struct Box<Value : P & Q & R & S & T> : P & Q & R & S & T {
  let value: Value
}
...
// CHECK:   call swiftcc void @"$s4main7consumeyyxlF"(%swift.opaque* noalias nocapture %{{[0-9]+}}, %swift.type* getelementptr inbounds (%swift.full_type, %swift.full_type* bitcast (<{ i8**, [[INT]], %swift.type_descriptor*, %swift.type*, i8**, i8**, i8**, i8**, i8**, i32{{(, \[4 x i8\])?}}, i64 }>* @"$s4main5ValueVyAA3BoxVySiGGMf" to %swift.full_type*), i32 0, i32 1))
// CHECK: }
func doit() {
...
// CHECK:   [[EQUAL_TYPE:%[0-9]+]] = icmp eq i8* bitcast (%swift.type* getelementptr inbounds (%swift.full_type, %swift.full_type* bitcast (<{ i8**, [[INT]], %swift.type_descriptor*, %swift.type*, i8**, i8**, i8**, i8**, i8**, i32{{(, \[4 x i8\])?}}, i64 }>* @"$s4main3BoxVySiGMf" to %swift.full_type*), i32 0, i32 1) to i8*), %"load argument at index 0 from buffer"
...
// CHECK:   ret %swift.metadata_response { %swift.type* getelementptr inbounds (%swift.full_type, %swift.full_type* bitcast (<{ i8**, [[INT]], %swift.type_descriptor*, %swift.type*, i8**, i8**, i8**, i8**, i8**, i32{{(, \[4 x i8\])?}}, i64 }>* @"$s4main5ValueVyAA3BoxVySiGGMf" to %swift.full_type*), i32 0, i32 1), [[INT]] 0 }
// CHECK: [[EXIT_NORMAL]]:
